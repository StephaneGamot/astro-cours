...
export default async function NotFound() {
  ...
  const SECTIONS = [
    ...
    {
      title: t("sec.blogTitle"),
      href: "/blog",
      description: t("sec.blogDesc"),
      color: "text-rose-400",
      border: "border-rose-500/20 hover:border-rose-500/40",
      bg: "hover:bg-rose-500/5",
    },
    {
      title: t("sec.dictTitle"),
      href: "/dictionnaire-astrologique",
      description: t("sec.dictDesc"),
  ...
  return (
    ...
    >
      ...
      <h1 className="mb-4 font-serif text-4xl text-white md:text-5xl">{t("h1")}</h1>
      <p className="mb-12 max-w-md text-lg leading-relaxed text-slate-400">{t("intro")}</p>
          ...
          <ul className="grid gap-3 sm:grid-cols-2">
            {SECTIONS.map((section) => (
              <li key={section.href}>
                ...
                >
                  <span className={`text-sm font-semibold ${section.color} transition-colors`}>
                    {section.title}
                  </span>
                  <span className="mt-1 text-sm text-slate-400">{section.description}</span>
                </Link>
              </li>
            ))}
          </ul>
