...
/* ------------------------------------------------------------------ */
/*  Locale-aware loading (overlay data/<locale>/ sur base FR)          */
/*  Repli gracieux par maison/planète : tant que non traduite, FR.    */
/* ------------------------------------------------------------------ */

const houseCache = new Map<string, House[]>();
const planetCache = new Map<string, Planet[]>();

function overlay<T extends { slug: string }>(base: T[], file: string): T[] {
  try {
    const loc = JSON.parse(fs.readFileSync(file, "utf-8")) as T[];
    const bySlug = new Map(loc.map((x) => [x.slug.toLowerCase(), x]));
    return base.map((x) => bySlug.get(x.slug.toLowerCase()) ?? x);
  } catch {
    return base;
  }
}

export function getHousesForLocale(locale?: string): House[] {
  if (!locale || locale === "fr") return HOUSES;
  const cached = houseCache.get(locale);
  if (cached) return cached;
  const merged = overlay(
    HOUSES,
    path.join(process.cwd(), "data", locale, "maisons.details.json")
  );
  houseCache.set(locale, merged);
  return merged;
}

export function getPlanetsForLocale(locale?: string): Planet[] {
  if (!locale || locale === "fr") return PLANETS;
  const cached = planetCache.get(locale);
  if (cached) return cached;
  const merged = overlay(
    PLANETS,
    path.join(process.cwd(), "data", locale, "planetes.details.json")
  );
  planetCache.set(locale, merged);
  return merged;
}
...
export function getHouse(slug: string, locale?: string): House | undefined {
  const s = decodeURIComponent(slug).trim().toLowerCase();
  return getHousesForLocale(locale).find((h) => h.slug === s);
}

export function getHouseIndex(slug: string, locale?: string): number {
  return getHousesForLocale(locale).findIndex((h) => h.slug === slug);
}
...
export function buildBreadcrumbs(
  house: House,
  labels?: { home?: string; houses?: string }
) {
  return [
...
export function planetInHouseTitle(
  planet: Planet,
  house: House,
  fallback?: string
): string {
  ...
  return fallback ?? `${planet.name} en Maison ${toRoman(house.numero)}`;
}
