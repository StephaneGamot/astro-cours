...
export function HouseNav({
  ...
  ring,
  labels = {
    nav: "Navigation entre les maisons",
    prev: "Précédent",
    next: "Suivant",
    prevAria: "",
    nextAria: "",
    ...
    >
      {prev ? (
        <Link
          ...
          aria-label={labels.prevAria || `${labels.prev} : ${prev.titreCourt}`}
        >
          <div className="flex items-center gap-4">
            <div className="min-w-0 flex-1">
              <p className="text-xs uppercase tracking-wide text-muted">{labels.prev}</p>
      ...
      {next ? (
        <Link
          ...
          className={`group rounded-3xl border ${border} bg-white/[0.02] p-5 transition hover:bg-white/[0.06] focus:outline-none focus-visible:ring-2 ${ring}`}
          aria-label={labels.nextAria || `${labels.next} : ${next.titreCourt}`}
        >
          <div className="flex items-center gap-4">
            <div
              className={`relative h-16 w-24 shrink-0 overflow-hidden rounded-2xl border ${border} bg-white/5`}
            ...
            <div className="min-w-0 flex-1 text-right">
              <p className="text-xs uppercase tracking-wide text-muted">{labels.next}</p>
              <p className="mt-1 font-serif text-xl group-hover:underline underline-offset-4 decoration-white/30">
