...
/*  Base FR + overlay data/<locale>/planetes.details.json par slug.    */
/*  Repli gracieux par planète : tant qu'une planète n'est pas dans    */
/*  le fichier de la locale, on garde sa version FR.                   */
/* ------------------------------------------------------------------ */

const localeCache = new Map<string, Planet[]>();
...
export function getPlanetsForLocale(locale?: string): Planet[] {
  ...
  let merged = PLANETS;
  try {
    const fp = path.join(process.cwd(), "data", locale, "planetes.details.json");
    const loc = JSON.parse(fs.readFileSync(fp, "utf-8")) as Planet[];
    const bySlug = new Map(loc.map((p) => [p.slug.toLowerCase(), p]));
    merged = PLANETS.map((p) => bySlug.get(p.slug.toLowerCase()) ?? p);
  } catch {
    /* pas de fichier de locale → tout FR */
  }
  localeCache.set(locale, merged);
  return merged;
}

export function getPlanet(slug: string, locale?: string): Planet | undefined {
  return getPlanetsForLocale(locale).find(
    (p) => p.slug.toLowerCase() === slug.toLowerCase()
  );
}

export function getPlanetIndex(slug: string, locale?: string): number {
  return getPlanetsForLocale(locale).findIndex(
    (p) => p.slug.toLowerCase() === slug.toLowerCase()
  );
}
...
export function buildBreadcrumbs(
  planet: Planet,
  labels?: { home?: string; planets?: string }
) {
  return [
    { name: labels?.home ?? "Accueil", href: "/" },
    { name: labels?.planets ?? "Planètes", href: "/planetes" },
