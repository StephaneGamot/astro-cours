...
import { revolutionSolaireContent } from "./content";

/**
 * Page « Révolution solaire » — cours premium, i18n (fr/en/es).
 * Tout le texte vient de ./content.tsx ; ici uniquement la présentation.
 */

const INTERNAL_PATH = "/revolution-solaire";
...
export async function generateMetadata({
  ...
  const c = revolutionSolaireContent[loc];
  return buildMeta({
    title: c.meta.title,
...
export default async function RevolutionSolairePage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const faqJsonLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: c.detailsFaqJsonLd.map((item) => ({
  ...
  return (
    <>
      ...
      <main className="relative mx-auto max-w-7xl px-6 pb-24 text-slate-200 selection:bg-amber-500/30" id="main-content">
        ...
        {/* SOMMAIRE */}
        <nav aria-label={c.navLabel} className="mb-20 flex flex-wrap justify-around md:justify-center gap-3">
          {c.sectionsNav.map((s) => (
        ...
        <section className="max-w-6xl mx-auto">
          ...
          <div className="grid gap-8 lg:grid-cols-2">
            <div className={`${card} ${accent("method").border} ${accent("method").glow}`}>
              ...
              <h3 className="font-serif text-2xl text-white">{c.methode.reglesOrTitle}</h3>
              <ul className="mt-6 space-y-4 text-[15px] leading-relaxed text-slate-300">
                {c.methode.reglesOr.map((x) => (
                  <li key={x} className="flex gap-3">
                    <span className={`${dot} ${accent("method").dot}`} />
                    <span>{x}</span>
                  </li>
            ...
            <div className={`${card} ${accent("moon").border} ${accent("moon").glow}`}>
              ...
              <h3 className="font-serif text-2xl text-white">{c.methode.checklistTitle}</h3>
              <ul className="mt-6 space-y-4 text-[15px] leading-relaxed text-slate-300">
                {c.methode.checklist.map((x) => (
                  <li key={x} className="flex gap-3">
          ...
          <div className={`${softCard} mt-8 ${accent("sun").border} ${accent("sun").glow}`}>
            ...
            >
              <Sun size={14} />
              {c.methode.orbesLabel}
            </div>

            <div className="grid gap-4 sm:grid-cols-2">
              {c.methode.orbesRecommandes.map((o) => (
                <div
                  key={o.label}
        ...
        <section className="mt-16 space-y-6" aria-labelledby="faq-revolution-solaire">
          <h2 id="faq-revolution-solaire" className="font-serif text-2xl sm:text-3xl">{c.faqVisibleTitle}</h2>
          <div className="space-y-4">
            {c.detailsFaq.map((item, i) => (
              <details
                key={i}
              ...
              >
                ...
                <p className="px-6 pb-6 text-sm leading-relaxed text-white/75 md:text-base">{item.a}</p>
              </details>
            ))}
          </div>
        </section>
