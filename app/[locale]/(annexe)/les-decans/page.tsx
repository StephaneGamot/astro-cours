...
import { decansContent } from "./content";
...
/**
 * Page « Les décans » — cours pro, i18n (fr/en/es).
 * Tout le texte vient de ./content.tsx ; ici uniquement la présentation.
 */

const INTERNAL_PATH = "/les-decans";
...
// ==========================================
// 2. HELPERS VISUELS (COULEURS PLANÈTES)
// ==========================================
...
// ==========================================
export default async function DecansPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
    ...
    <>
      ...
      <main className="relative mx-auto max-w-7xl px-6 pb-24 text-slate-200 selection:bg-amber-500/30" id="main-content">
        ...
        <header className="relative mb-28 pt-16 text-center md:text-left">
          <div className="flex flex-col lg:flex-row gap-16 items-center">
            ...
            <div className="relative w-full max-w-lg aspect-square rounded-[3rem] overflow-hidden border border-white/10 shadow-[0_0_50px_rgba(251,191,36,0.1)] bg-[#0f0f13] flex items-center justify-center">
              <Image src={DecansHero} alt={c.hero.heroImageAlt} fill priority className="object-cover" />
        ...
        <p className="mt-8 max-w-3xl text-base leading-relaxed text-white/75 sm:text-lg">
          <strong>{c.appIntro.tail}</strong> {c.appIntro.lead}
        </p>
        ...
        <section className="mb-32 grid gap-8 lg:grid-cols-2 max-w-6xl mx-auto">
          <div className="relative overflow-hidden rounded-[2.5rem] border border-white/10 bg-[#0c0c0e] p-10 backdrop-blur-md transition-all hover:border-white/20">
            ...
            <h2 className="relative z-10 font-serif text-3xl text-white mb-8 border-b border-white/10 pb-4">{c.histoireTitle}</h2>
          ...
          <div className="relative overflow-hidden rounded-[2.5rem] border border-amber-500/20 bg-gradient-to-br from-amber-500/5 to-transparent p-10 backdrop-blur-md transition-all hover:border-amber-500/40">
            ...
            <h3 className="relative z-10 font-serif text-3xl text-white mb-8 border-b border-amber-500/20 pb-4">{c.matriceTitle}</h3>
            <div className="relative z-10 space-y-6 text-[15px] md:text-base leading-relaxed text-slate-300">
        ...
        <section className="max-w-6xl mx-auto">
          <SectionTitle id="utilisations" icon={Layers}>{c.utilisationsTitle}</SectionTitle>
          <p className="mb-12 text-lg text-slate-300 font-light italic max-w-4xl leading-relaxed">{c.utilisationsIntro}</p>

          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            {c.utilisations.map((item, i) => (
          ...
          <div className="mt-16 mb-16 relative w-full h-[350px] md:h-[450px] rounded-[3rem] border border-white/10 overflow-hidden bg-[#111] flex items-center justify-center shadow-2xl">
              <Image src={DecansRoue} alt={c.roueImageAlt} fill className="object-cover" />
        ...
        <section>
          <SectionTitle id="encyclopedie" icon={BookOpen}>{c.signesTitle}</SectionTitle>
          <p className="mb-16 text-lg text-slate-300 font-light italic max-w-5xl leading-relaxed">{c.signesIntro}</p>

          <div className="space-y-24">
            {c.signes.map((signe, idx) => (
              <div key={idx} className="scroll-mt-32" id={`signe-${signe.id}`}>
                {/* En-tête du Signe */}
                      ...
                      <article key={decan.numero} className="relative flex flex-col rounded-[2.5rem] border border-white/5 bg-[#0c0c0e] p-8 shadow-xl transition-all hover:bg-[#111] hover:border-white/15">
                        ...
                        <div className="flex items-center justify-between mb-6">
                          ...
                          <span className={`px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest border ${stylePlanete}`}>
                            {c.planetLabel[decan.planete] ?? decan.planete}
                          </span>
                        </div>
                        ...
                        <div className="space-y-4 pt-6 border-t border-white/5">
                          <div>
                            <span className="text-[10px] uppercase tracking-widest text-emerald-500 font-bold block mb-1">{c.decanCard.forcesLabel}</span>
                            <p className="text-sm text-slate-400 leading-relaxed">{decan.facilites}</p>
                          </div>
                          <div>
                            <span className="text-[10px] uppercase tracking-widest text-rose-500 font-bold block mb-1">{c.decanCard.defisLabel}</span>
            ...
            {c.faq.map((f, i) => (
              ...
              >
                ...
                <p className="px-6 pb-6 text-sm leading-relaxed text-white/75 md:text-base">
                  {f.a.lead}
                  {f.a.link ? <Link href={f.a.link.href}>{f.a.link.label}</Link> : null}
                  {f.a.tail}
                </p>
              </details>
            ))}
